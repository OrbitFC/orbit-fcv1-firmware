...
void bno055_setOperationMode(bno055_opmode_t mode) { //page 21 for details 
    bno055_writeData(BNO055_OPR_MODE, mode);
    if (mode == BNO055_OPERATION_MODE_CONFIG) {
        bno055_delay(19); //time required for switching from any op mode to CONFIG mode
    } else {
        bno055_delay(7); //time required for switch from CONFIG mode to any op mode
    }
}
...
HAL_StatusTypeDef bno055_writeData(uint8_t reg, uint8_t data) {
